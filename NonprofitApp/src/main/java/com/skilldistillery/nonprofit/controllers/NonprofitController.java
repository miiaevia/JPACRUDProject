...
public class NonprofitController {
	...
	public ModelAndView addNonprofitPost(Nonprofit newNP, RedirectAttributes redir) {
		ModelAndView mv = new ModelAndView();
		Nonprofit np = dao.create(newNP);
		if (np != null) {
			mv.addObject(np);
			mv.addObject("nonprofit", np);
			redir.addFlashAttribute("nonprofit", np);
			// mv.setViewName("WEB-INF/views/nonprofit.jsp");
			mv.setViewName("redirect:addNPRedir.do");
		} else {
	...
	@RequestMapping(path = "addNPRedir.do", method = RequestMethod.GET)
